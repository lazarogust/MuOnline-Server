...
void Draw_RenderObject(OBJECT *o,bool Translate,int Select, int ExtraMon)
{
    ...
    if((EditFlag!=EDIT_NONE) || (EditFlag==EDIT_NONE&&o->HiddenMesh!=-2))
	{
		if (ExtraMon != 10)
		{
			if (o->Type >= gItemManager.GET_ITEM_MODEL(18, 0) && o->Type <= gItemManager.GET_ITEM_MODEL(18, 254) || o->Type == 349) 
			{
				BMD* b = &Models[o->Type];
			...
			else if (gHelperSystem.CheckIsHelper(o->Type))
			{
				BMD* b = &Models[o->Type];
			...
			else if (o->Type >= gItemManager.GET_ITEM_MODEL(16, 255) && o->Type < gItemManager.GET_ITEM_MODEL(17, 0))
			{
				BMD* b = &Models[o->Type];
...
void CreateItem(ITEM_t *ip,BYTE *Item,vec3_t Position,int CreateFlag)
{
	...
	else
	{
		...
		if(CreateFlag)
		{
			if(Type==ITEM_POTION+13 || Type==ITEM_POTION+14 ||  Type==ITEM_POTION+16 || Type==ITEM_WING+15 || Type==ITEM_POTION+22 || Type==ITEM_POTION+31 || gCustomJewel.CheckCustomJewelByItem(Type))
...
void RenderPartObjectBody(BMD *b,OBJECT *o,int Type,float Alpha,int RenderType)
{
	...
	if (gRenderModel.RenderModelBodyLua(Type, BMDStruct, o) == 1)
	{
		return;
	}
...
void RenderPartObjectBodyColor(BMD *b,OBJECT *o,int Type,float Alpha,int RenderType,float Bright,int Texture, int iMonsterIndex)
{
	...
	{
		gItemManager.GetItemColor(Type,Alpha,Bright,b->BodyLight, (RenderType&RENDER_EXTRA) ? true : false );
	}
	...
	if ( Type==MODEL_MACE+7 || gRenderModel.RenderModelBodyColorLua(Type, b, o) == 1)
	{
		o->HiddenMesh = -1;
	}
}
...
void RenderPartObjectBodyColor2(BMD *b,OBJECT *o,int Type,float Alpha,int RenderType,float Bright,int Texture)
{
	...
	if (gRenderModel.RenderModelBodyColor2Lua(Type, b, o) == 1)
	{
		return;
	}
...
{	
	...
	if (gCustomWing.CheckCustomWingByItem(Type - MODEL_ITEM))
	{
		Level = 0;
	}
	...
	if(!o->EnableShadow)
	{
		...
		if (gDisableExcellent.CheckDisableExcellent(Type) == 0)
		{
			...
			{
				if ((Option1 & 63) > 0 && (o->Type<MODEL_WING || o->Type>MODEL_WING + 6) && o->Type != MODEL_HELPER + 30 && (o->Type<MODEL_WING + 36 || o->Type>MODEL_WING + 43) && (o->Type < MODEL_WING + 130 || MODEL_WING + 134 < o->Type))
				{
