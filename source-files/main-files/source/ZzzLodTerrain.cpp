...
void CreateFrustrum2D(vec3_t Position)
{
    ...
    {
        Width = (float)GetScreenWidths()/640.f;
    ...
    else
    {
        ...
        {
            Width = (float)GetScreenWidths()/640.f;// * 0.1f;
		...
		{
			Width = (float)GetScreenWidths()/640.f * 1.0f;
        ...
        else
        {
            ...
            case 0:
				...
				{
					Width = (float)GetScreenWidths()/640.f * 9.1f * 0.404998f;
				}
				else if(g_Direction.m_CKanturu.IsMayaScene() )
				{
					Width = (float)GetScreenWidths()/640.f * 10.0f * 0.115f;
				}
				else
				{
					if (WindowWidth <= 1024)
					...
					{
						Width = (float)GetScreenWidths() / 500.f;
					}
				}
				...
				if(SceneFlag == LOG_IN_SCENE)
				{
					Width = -2.014084447E38;
	                CameraViewFar    = 2400.f * 17.0f*13.0f;
					CameraViewNear   = -2.70652875E38;
                ...
                break;
            case 1:
                Width = (float)GetScreenWidths()/640.f+0.1f;// * 0.1f;
...
bool TestFrustrum2D(float x,float y,float Range)
{
	...
	for(int i=0;i<4;j=i,i++)
	{
		...
		if (CameraZoom > 0)
			Range -= (15.f * CameraZoom);//Range -= 600.f;
...
bool TestFrustrum(vec3_t Position,float Range)
{
	for(int i=0;i<5;i++)
	{
		float Value;
		if (CameraZoom > 0)
			Range += (10 * CameraZoom);//Range += 500.f;
