...
void RenderNotices()
{
	...
	for(int i=0;i<MAX_NOTICE;i++)
	{
		...
		{
			g_pRenderText->RenderText(setPosCenterX(0), (GetWindowsY - g_pChatInputBox->m_posy) + i * 13, n->Text, 0, 0, RT3_WRITE_CENTER);
		}
		else
		{
			g_pRenderText->RenderText(setPosCenterX(0), (GetWindowsY - g_pChatInputBox->m_posy) + i * 13, n->Text, 0, 0, RT3_WRITE_CENTER);
			...
			if(strcmp(Name,GlobalText[258])==NULL || strcmp(Name,GlobalText[259])==NULL || stricmp(Name,"/trade")==NULL)
			{
				...
				if(SelectedCharacter!=-1)
				{
						...
						abs(( c->PositionY)-( Hero->PositionY))<=1)
					{
						...
						SendRequestTrade(CharactersClient[SelectedCharacter].Key, 0);
					}
				}
			...
			if (stricmp(Name, "/tradex") == NULL)
			{
				...
				if (SelectedCharacter != -1)
				{
						...
						abs((c->PositionY) - (Hero->PositionY)) <= 1)
					{
						...
						SendRequestTrade(CharactersClient[SelectedCharacter].Key, 1);
					}
				}
				else for (int i = 0; i < MAX_CHARACTERS_CLIENT; i++)
				{
            ...
            case AT_SKILL_THUNDER_STRIKE:
                if ( CheckTile( c, o, Distance ) )
                {
					if (c->Helper.Type == MODEL_HELPER + 4 || gHelperSystem.CheckHelperType(c->Helper.Type, 16) == 1 || !c->SafeZone)
					{
						...
						{
							if (c->Helper.Type == MODEL_HELPER + 4 && !c->SafeZone || gHelperSystem.CheckHelperType(c->Helper.Type, 16) == 1 && !c->SafeZone)
							{
								SetAction(o, PLAYER_ATTACK_RIDE_ATTACK_FLASH);
							}
							else if (c->Helper.Type == MODEL_HELPER + 37 && !c->SafeZone || gHelperSystem.CheckHelperType(c->Helper.Type, 8) == 1 && !c->SafeZone)
							{
				...
				if(( LoadingWorld)==0 && Hero->JumpTime==0)
				{
					...
					if(Success)
					{
                            ...
                            ( ( CharacterMachine->Equipment[EQUIPMENT_HELPER].Type>=ITEM_HELPER+2  && CharacterMachine->Equipment[EQUIPMENT_HELPER].Type<=ITEM_HELPER+3 ) ) )
						{
							...
							|| (gCustomWing.CheckCustomWingByItem(CharacterMachine->Equipment[EQUIPMENT_WING].Type))))
						{
...
void SelectObjects()
{
	...
	if(!MouseOnWindow && false == g_pNewUISystem->CheckMouseUse() && SEASON3B::CheckMouseIn(0, 0, GetWindowsX, GetWindowsY - 51))
	{
		if(HIBYTE(GetAsyncKeyState(VK_MENU))==128)
		{
...
void RenderOutSides()
{
	if(gMapManager.WorldActive == WD_8TARKAN)
	{
		...
		RenderBitmapUV(BITMAP_CHROME+2,0.f,0.f,640.f + GWidescreen.DisplayWinCDepthBox, GetWindowsY,WindX,0.f,0.3f,0.3f);
		float WindX2 = (float)((int)WorldTime%100000)*0.001f; 
		RenderBitmapUV(BITMAP_CHROME+3,0.f,0.f,640.f + GWidescreen.DisplayWinCDepthBox, GetWindowsY,WindX2,0.f,3.f,2.f);
	}
	...
	else if (IsKarutanMap())
	{
		...
		RenderBitmapUV(BITMAP_CHROME+3, 0.f, 0.f, 640.f + GWidescreen.DisplayWinCDepthBox, 480.0, fWindX, 0.f, 3.f, 2.f);
	}
