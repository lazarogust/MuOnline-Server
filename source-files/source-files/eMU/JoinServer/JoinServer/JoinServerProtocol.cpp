...
void JoinServerProtocolCore(int index,BYTE head,BYTE* lpMsg,int size) // OK
{
	...
	switch(head)
	{
		...
		case 0x30:
			...
			break;
		case 0x2F:
			GJServerMessageInfoRecv((SDHP_SERVER_MESSAGE_INFO_RECV*)lpMsg, index);
			break;
		case 0x40:
...
void GJServerMessageInfoRecv(SDHP_SERVER_MESSAGE_INFO_RECV* lpMsg, int index)
{
	SDHP_SERVER_MESSAGE_INFO_SEND pMsg;

	pMsg.header.set(0x2F, sizeof(pMsg));

	strcpy_s(pMsg.Msg, lpMsg->Msg);
	...
	for (int n = 0; n < MAX_SERVER; n++)
	{
		...
		{
			gSocketManager.DataSend(gServerManager[n].m_index, (BYTE*)&pMsg, pMsg.header.size);
		}
	}
}
