...
--when player clicks the buy button on warehouse
local RequestWarehouseCost_Handles = {}

function RequestWarehouseCost(aIndex)
	for i = 1, #RequestWarehouseCost_Handles do
		RequestWarehouseCost_Handles[i].callback(aIndex)
	end
end

function GameServerFunctions.RequestWarehouseCost(callback)
	table.insert(RequestWarehouseCost_Handles, { callback = callback })
end


--when player wants to buy another warehouse
local PlayerBuyVaultOk_Handles = {}

function PlayerBuyVaultOk(aIndex)
	for i = 1, #PlayerBuyVaultOk_Handles do
		PlayerBuyVaultOk_Handles[i].callback(aIndex)
	end
end

function GameServerFunctions.PlayerBuyVaultOk(callback)
	table.insert(PlayerBuyVaultOk_Handles, { callback = callback })
end
...
function PlayerDropItem(aIndex, x, y, slot)
	for i = 1, #PlayerDropItem_Handles do
		if PlayerDropItem_Handles[i].callback(aIndex, x, y, slot) == 1 
		then
			return 1
		end
	end
...
function PlayerOpenShop(aIndex)
	...
	for i = 1, #PlayerOpenShop_Handles do
		if PlayerOpenShop_Handles[i].callback(aIndex) == 1
		then
			return 1
		end
	end
...
function PlayerCloseShop(aIndex)
	for i = 1, #PlayerCloseShop_Handles do
		if PlayerCloseShop_Handles[i].callback(aIndex) == 1
		then
			return 1
		end
	end
...
function MacAddressPlayer(aIndex, HWID)
	...
	DataBase.SetString("MEMB_STAT", "HWID", HWID, "memb___id", player:getAccountID())
